...
const fillModel = async (equipment, character) => {
  //will probably need a separate way to update an existing model
  ...
  let health =
    character.class.hit_die + ability_relevant.ability_scores.con.modifier; //only works for 1st level
  if (character.subrace == 'Hill Dwarf') health += level;
  let subclassFeatures = character.subclass
    ? character.subclass.subclass_features
    : [];
  ...
  const model = {
    ...
    ac: acCalculator(
      ...
      ability_relevant.ability_scores
    ),
    health: {
      //not expandable for multiple subraces...it's checking for hill dwarf lazily
      current: health,
      max: health,
      temp: 0,
    },
    hit_dice: [
      //for multiclassing will need to populate further
      {
        current: level,
        max: level,
    ...
    spells: character.spells
      ? {
          ...
          casting_ability:
            character.class.spellcasting.spellcasting_ability.index,
          advantage: 0,
...
const abilityScoreParser = (
  ...
  for (let ability in abilities) {
    ...
    for (let skill of skillScores[ability.short_name]) {
      //console.log(skill);
      let proficiencyExists = allSkillProfs.find(
        aSkill => aSkill.toLowerCase().replaceAll(' ', '_') === skill
      );
      let expertise = selections.expertise.find(
        aExp =>
          aExp
            .toLowerCase()
      ...
      scores.skills[skill] = {
        ...
        modifier: !expertise
          ? ability.modifier + (!proficiencyExists ? 0 : proficiency)
          : ability.modifier + proficiency * 2,
        advantage: 0,
      };
    }
    ...
    let proficiencyExists = allThrowProfs.find(
      aThrow => aThrow.toLowerCase() === ability.short_name
    );
    scores.saving_throws[ability.short_name] = {
      proficiency: !proficiencyExists ? false : true,
      modifier: !proficiencyExists
        ? ability.modifier
    ...
    let maxAC =
      ...
      (armor.armor_class.dex_bonus
        ? armor.armor_class.max_bonus &&
          abScores.dex.modifier >= armor.armor_class.max_bonus
          ? armor.armor_class.max_bonus
          : abScores.dex.modifier
        : 0);
