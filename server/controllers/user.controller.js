...
const attachUserInfo = (req, res, next) => {
  User.findById(req.locals.auth.id, projections.all)
    .then(user => {
...
const user_detail = (req, res) => {
  ...
  User.find({ username: req.params.username }, projection)
    .then(user => {
      if (!user) return throwError(res, 401, 'No Content.');
  ...
  User.findOne(query)
    .then(user => {
      ...
      bcrypt
        ...
        .then(response => {
          if (response === false)
            return throwError(res, 401, 'Invalid credentials.');
          ...
          jwt.sign(jwtPayload, config.jwt.secret, (error, token) => {
            if (error)
              return throwError(res, 401, 'Invalid credentials.', error);
            /*
            ...
            return res
              .status(200)
              .cookie('auth', token, config.jwt.options)
              .json(user);
          });
        })
...
const logout_user = (req, res) => {
  res
    .clearCookie('auth')
    .status(200)
    .send('Successfully logged out. Cookie cleared.');
};
