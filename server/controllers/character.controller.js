...
const attachCharacterInfo = (req, res, next, id) => {
    ...
    Character.findById(id, projection).then(character => {
...
const character_list = (req, res) => {
    ...
    if(query){
        Character.find(query, projection).then(characters => {
...
const delete_character = (req, res) => {
    Character.findByIdAndDelete(req.params.characterid).then(deletedCharacter => {
        if(deletedCharacter){
